#Usage: $ python predDTI_NR.py NR_Target.fasta Drug_selFV.csv > predDTI_NR.out
...
XGBpram = ['Fold1.csv', 'Fold2.csv', 'Fold3.csv', 'Fold4.csv', 'Fold5.csv']

if len(allSeqName) == len(queryX):
  predY = []
  for i in XGBpram:
    nameF = i.split('_')[0]
    NEval = int(i.split('_')[1])
    LRval = float(i.split('_')[2])
    ...
    clf = XGBClassifier(n_estimators=NEval, learning_rate=LRval)
